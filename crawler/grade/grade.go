...
func GetGradeFromXK(sid, password string, curRecordNum int) (*[]ResultGradeItem, bool, error) {
	...
	if err := json.Unmarshal(body, &data); err != nil {
		log.Println("Json unmarshal failed, maybe login error, need to enter verification code", err)
		return nil, false, err
	}
	...
	for _, d := range *data.Items {
		...
		if err != nil {
			fmt.Println("GetUsualAndFinalGradeFromXK function error for ", d.Kch, d.Kcmc, d.Jsxm)
			continue
		}
		t, err := strconv.ParseFloat(d.Cj, 32)
		if err != nil {
			log.Printf("parse %s to float error: %t", d.Cj, err)
			continue
		}
	...
	log.Println("Get grades successfully")
